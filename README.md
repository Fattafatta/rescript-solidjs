...
The idea is to use [babel](https://babeljs.io/) to transform compiled ReScript code back to `JSX`. This is done by a plugin that replaces all occurrences of `React.createElement` (and similar functions produced by ReScript) with `JSX`. The corresponding plugin can be found here: <https://github.com/Fattafatta/babel-plugin-rescript-react-to-jsx>
...
| Context | Supports Context | `HyperScript` creates all components "inside out". This means the child components are created first. This makes ContextProviders unusable. |
...
Follow the instructions in the [README](https://github.com/Fattafatta/babel-plugin-rescript-react-to-jsx/blob/main/README.md) to configure `babel`.
